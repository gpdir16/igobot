...
export async function compressContext(messages, llmClient, keepRecent = 20) {
  ...
  try {
    const response = await llmClient.chat({
      instructions: '당신은 대화 요약 전문가입니다. 제공된 대화 내용을 핵심만 간결하게 요약하세요. 중요한 정보(이름, 숫자, 결정사항, 코드 등)는 반드시 포함하세요. 한국어로 작성하세요.',
      messages: [
        { role: 'user', content: `다음 대화 내용을 핵심만 요약해주세요:\n\n${summaryInput}` }
      ],
      tools: []
    });
    ...
    // 압축 실패 시 단순 잘라내기
    return [
      { role: 'assistant', content: '[이전 대화가 길어서 일부가 생략되었습니다.]' },
      ...toKeep
    ];
  }
}
