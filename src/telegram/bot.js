...
class TelegramBot {
  ...
  async start(messageHandler) {
    ...
    this.bot.action(/^toollog:(.+)$/, async ctx => {
      ...
      try {
          ...
          const sent = await ctx.telegram.sendMessage(ctx.chat.id, header + logChunks[i], {
            ...
            ...(isLast ? {
              reply_markup: Markup.inlineKeyboard([
                Markup.button.callback('🗑 닫기', `closelog:${id}`)
              ]).reply_markup
            } : {})
          });
        ...
        try { await ctx.telegram.sendMessage(ctx.chat.id, `⚠️ 기록 표시 실패: ${err.message}`); } catch {}
      }
    });
  ...
  async sendTyping(chatId) {
    if (!this.bot) return;
    try { await this.bot.telegram.sendChatAction(chatId, 'typing'); } catch {}
  }
  ...
  async sendPhoto(chatId, photoSource, caption = '') {
    if (!this.bot) return;
    // 로컬 파일 경로면 스트림으로 변환
    const source = (typeof photoSource === 'string' && !photoSource.startsWith('http') && existsSync(photoSource))
  ...
  async sendDocument(chatId, docSource, fileName = '', caption = '') {
    if (!this.bot) return;
    // 로컬 파일 경로면 스트림으로 변환
    const source = (typeof docSource === 'string' && !docSource.startsWith('http') && existsSync(docSource))
  ...
  async sendWithToolLog(chatId, text, toolHistory) {
    ...
    for (let i = 0; i < textChunks.length; i++) {
      ...
      const msg = await this.bot.telegram.sendMessage(chatId, textChunks[i] || '(응답 없음)', {
        ...
        ...(isLast ? {
          reply_markup: Markup.inlineKeyboard([
            Markup.button.callback('📋 도구 사용 기록', `toollog:${logId}`)
          ]).reply_markup
        } : {})
      });
      lastMsgId = msg.message_id;
